...
class Criminal:
    def __init__(self,root):
        ...
        upper_frame=LabelFrame(Main_frame, bd=2,relief=RIDGE,text='Criminal Imformation',font=('times new roman',11, 'bold'),fg='red' ,bg='white')
        upper_frame.place(x=10, y=10,width=1480, height=270)

        down_frame=LabelFrame(Main_frame, bd=2,relief=RIDGE,text='Criminal Imformation Table',font=('times new roman',11, 'bold'),fg='red' ,bg='white')
        ...
        txt_occupution=ttk.Entry(upper_frame,width=22,textvariable=self.var_occupation, font=("arial",11,"bold"))
        ...
        # Clear button
        btn_clear=Button(button_frame,command=self.clear_date, text='Clear',font=("arial",13,"bold"),width=14, bg='blue',fg='white')
        ...
        # Down frame
        down_frame=LabelFrame(Main_frame, bd=2,relief=RIDGE,text="Criminal Imformation Table",font=("arial",13,"bold"))
        down_frame.place(x=10, y=280,width=1480, height=270)

        search_frame=LabelFrame(down_frame, bd=2,relief=RIDGE,text="Search Criminal Record",font=("arial",13,"bold"))
    ...
    def add_data(self):
        if self.var_case_id.get()=="":
            ...
            try:
                connection=mysql.connector.connect(host='localhost' ,username='root' ,password='gtavcbtp2000' ,database='management')  
        ...
        if self.var_case_id.get()=="":
           messagebox.showerror('Error','All Fields are required')
            ...
            try:
                ...
                self.clear_date()
                connection.close()
                messagebox.showinfo('Success','Crimimal record successfully Updated ')
            except Exception as es:
                messagebox. showerror('Error',f'Due To{str(es)}')    
    # delete
        ...
        if self.var_case_id.get()=="":
           messagebox.showerror('Error','All Fields are required ')
            ...
            try:
                ...
                self.clear_date()
                connection.close()
                messagebox.showinfo('Success','Crimimal record successfully Deleted ')
            except Exception as es:
                messagebox.showerror('Error',f'Due To{str(es)}')
